...
Automatic classification is conservative. It selects `exact-edit` only for direct or exact replacement work and `tool-heavy` for debugging, tests, logs, traces, builds, linting, or noisy commands. Lookups and simple edits remain `general`. `--force` preserves the caller's requested level when the caller explicitly chooses to bypass evidence routing.

The validated Luna tool-heavy `full` route enables only Ranch. Baseline routes inject no Condiments policy. Active routes inject only their small control directive; when a native hook enforces Ranch, the Ranch prompt directive is also omitted.
