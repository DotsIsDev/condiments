...
The envelope preserves the tool name, request summary, exit status, original character and byte counts, SHA-256 hash, bounded previews, error lines, and absolute artifact path. Identical output reuses one artifact under `.condiments/artifacts/`. Repetitive logs also carry a lossless, hash-verified dictionary payload when the full encoded envelope fits the model-visible budget. See [log-dictionary.md](log-dictionary.md).
