...
import { interceptNativeToolOutput } from "../src/native-output.mjs";

async function main() {
  const options = parseOptions(process.argv.slice(2));
  const stdin = await readStdin();
  const payload = stdin.trim() ? JSON.parse(stdin) : {};
  const result = await interceptNativeToolOutput(payload, {
    host: options.host,
    cwd: options.cwd,
...
main().catch((error) => {
  process.stderr.write(`condiments-result-hook: ${error.message}\n`);
  process.exitCode = 2;
});
