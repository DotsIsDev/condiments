...
const executableName = packageJson.name.split("/").at(-1);
if (packageJson.bin?.[executableName] !== "scripts/install-adapter.mjs") {
...
if (errors.length) {
  ...
  process.stdout.write(`${JSON.stringify({ name: packageJson.name, version: packageJson.version, public: true, entry: packageJson.bin[executableName] })}\n`);
}
