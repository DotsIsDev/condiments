...
async function main() {
  ...
  const result = options.action === "ingest"
    ? await ingestMemoryEvent(root, input)
    : options.action === "query"
      ? await queryMemoryEvents(root, input)
      : decideMemoryConsolidation(input);
  process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
}

function parseOptions(args) {
  const action = args[0];
  if (!new Set(["ingest", "query", "decide"]).has(action)) throw new Error("Use ingest, query, or decide.");
...
main().catch((error) => {
  process.stderr.write(`condiments-zero-memory: ${error.message}\n`);
  process.exitCode = 2;
});
